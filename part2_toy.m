...
for frame = 1:31
    ...
    pttts3d(frame,:) = [P1',P2',P3',P4'];
    end
end
save('pttts3d.mat','pttts3d')
%% shadow planes
for frame = 1:31
    if isempty(find(pttts3d(frame,:) == 0))
        P1 = pttts3d(frame,1:3);
        P2 = pttts3d(frame,4:6);
        P3 = pttts3d(frame,7:9);
        P4 = pttts3d(frame,10:12);
        n_temp = cross((P2'-P1'),(P4'-P3'));
        n_hat = n_temp/norm(n_temp);
        ssshadow(frame,:) = [P3,n_hat'];
    end
end
save('ssshadow.mat','ssshadow')

%% Reconstruction

load('../data/cal/Calib_Results.mat');
frame_num = [];
frame_num = template(496:820,538:916); 
...
for x = 496:820
    for y = 538:916
        ...
        if (floor(template(x,y))==template(x,y))
            if template(x,y) ~=0
                P(k,:) = ssshadow(template(x,y),1:3);
                n(k,:) = ssshadow(template(x,y),4:6);
                lambda(k) = dot(n(k,:)',P(k,:)')/dot(n(k,:)',v);
                Ptttx(x-495,y-537) = lambda(k) * v(1);
                Pttty(x-495,y-537) = lambda(k) * v(2);
                Ptttz(x-495,y-537) = lambda(k) * v(3);
...
figure(10);
vv = VideoReader('DSC_0284.MOV');
frame1 = read(vv,1);
img = im2double(frame1);
colormat =img(496:820,538:916,:);
